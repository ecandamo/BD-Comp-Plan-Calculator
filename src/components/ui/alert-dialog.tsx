...
function AlertDialogOverlay({ className, ...props }: React.ComponentProps<typeof AlertDialogPrimitive.Overlay>) {
  return <AlertDialogPrimitive.Overlay data-slot="alert-dialog-overlay" className={cn("fixed inset-0 z-50 bg-slate-950/35 backdrop-blur-sm", className)} {...props} />;
}
  ...
  return (
    <AlertDialogPortal>
      ...
      <AlertDialogPrimitive.Content
        ...
        className={cn("fixed left-1/2 top-1/2 z-50 grid w-[min(96vw,520px)] -translate-x-1/2 -translate-y-1/2 gap-4 rounded-xl border border-border bg-popover p-6 text-popover-foreground shadow-lg", className)}
        {...props}
      />
    </AlertDialogPortal>
  );
